...
public class NotesService {
    ...
    public String restoreNote(String noteId) {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        String username  = auth.getName();
        User user = uRepo.findByUsername(username);
        ObjectId id = new ObjectId(noteId);
        Note note = nRepo.findById(id).orElse(null);
        if(user!=null && note!=null && user.getTrash().contains(note)){
            user.getTrash().remove(note);
    ...
    public String deleteTrashNote(String noteId) {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        String username  = auth.getName();
        User user = uRepo.findByUsername(username);
        ObjectId id = new ObjectId(noteId);
